  ...
  module Admin
    class BankDetailsController < Refinery::AdminController
      ...
      before_filter :find_available_plugins, :find_available_roles,
                    :only => [:add_client,:edit_client,:save_client,:update_client]
      ...
      before_filter :exclude_password_assignment_when_blank!, :only => :update
        ...
        @event_organizers = User.where('id in (?)',ids)
    	end
      ...
      def deactivate_client
        ...
        redirect_to '/refinery/admin/bank_details'
      end
